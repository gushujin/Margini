...
Versione Aggiornata: Layout Accorpato
"""
...
def alert_margine(acquisto: float, vendita: float):
    ...
    if margine < soglia:
        ...
        st.markdown(f'<div class="alert-box"><div style="color:var(--danger);font-weight:700">{T["alert_title"]}</div><div style="font-size:0.85rem">Margine: {margine*100:.2f}% (Min: {T["soglia_margine"]}%)<br>Acq. Max: {fmt_eur(acq_max)}</div></div>', unsafe_allow_html=True)
...
with tab_main:
    ...
    if acq1 and ven1:
        ...
        with cb: result_box(str(T["tab1_res2"]), fmt_pct((ven1/acq1-1)), "secondary")
    ...
    if sc_acq_raw and sc_ven_raw:
        ...
        if sa is not None and sv is not None:
            ...
            with cb: result_box(str(T["tab2_res2"]), fmt_pct(((1-sv)/(1-sa)-1)), "secondary")
    ...
    if li4 and ne4:
        result_box(str(T["tab4_res1"]), fmt_pct(1 - ne4/li4), "ok-val")

# ── TAB SCONTI MULTIPLI (Separato) ──
with tab_multi:
